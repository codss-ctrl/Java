...
public class Oop2_01 {
	public static void main(String[] args) {
		/*
		 ...
		 5. 오버라이드(Override)  		
		  	...
		  	- 조상클래스의 메서드를 자손클래스에서 오버라이드 할때는
		  	 	접근제어자를 조상클래스의 메서드보다 좁은 범위로 선택할 수 없다.    
		  	 	예외는  조상클래스의 메서드보다 많이 선언할 수 없다.
		  	 	인스턴스 메서드를 클래스 메서드로 변경할 수 없다.
		  	 	
		 6.	super.
		 	- 조상 클래스의 멤버와 자손클래스의 멤버가 중복정의되어 서로 구분해야하는 경우 사용
		 	- 자손클래스의 멤버에서만 사용이 가능
		 	- 조상의 멤버와 자신의 멤버를 구분하는데 사용된다는 점을 제외하고는 this.와 동일하다.
		 	- 클래스메서드에는 사용할 수 없고 인스턴스메서드에만 사용할 수 있다.
		  	 
		  	 
		 7. super()
		  	- this()와 마찬가지로 super() 역시 생성자 호출이다.
		  	- this()는 같은 클래스의 다른 생성자를 호출, super()는 자손클래스의 생성자에서 부모 클래스의 생성자를 호출할때 사용한다.
		 ...
		 8. 제어자
		  	...
		  	- 접근제어자와 그 외 제어자가 동시에 있을때 접근제어자를 가장 왼쪽에 쓴다. 
		  	- static(클래스의, 공통적인)
		  	 : 메서드와 변수, 초기화블럭에만 사용가능하다. 
		  	 : 클래스변수
		  	   -> 모든 인스턴스에서 공통적으로 사용
		...
		14. instanceof 	
		 	...
		 	(fc instanceoof Car)
		 	- 연산결과가 true 라면 참조변수가 검사한 타입으로 형변환이 가능하다.
		...
		16. 여러 객체 배열 Vector
			- 배열을 사용할 경우 전체 크기를 한번 정하면 변경이 불가하다.
			...
			- 동적으로 크기가 관리되는 객체배열 이라고 생각하면 된다.
		 	...
		 	- Vector 클래스의 메서드
		 	   ...
		 	   : remove() => 객체를 삭제한다.
		 	   : isEmpty() => Vector에 값이 비어 있는 지 확인한다.
		 	   : size() => 객체의 개수를 반환한다.
		...
		18. 인터페이스(Interface)
		 ...
		 19. 내부클래스
		 	- 클래스 내에 선언된 클래스이다.
		 	- 서로 쉽게 접근할 수 있고 외부에는 불필요한 클래스를 감춰서 복잡성을 줄일 수 있다. 
